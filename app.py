...
def recommend(S, close_series, composite, days):
    ...
    if TRADE_POLICY == "prefer_spread" and final["side"] in ("LONG_CALL", "LONG_PUT"):
        ...
        if is_call:
            long_p = bs_price_call(S, long_leg, R, Q, sigma, T); short_p = bs_price_call(S, short_leg, R, Q, sigma, T)
        else:
            long_p = bs_price_put(S, long_leg, R, Q, sigma, T); short_p = bs_price_put(S, short_leg, R, Q, sigma, T)
